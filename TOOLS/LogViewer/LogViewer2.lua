...
local m_index_file = require("LogViewer/index_file")
...
local function read_and_index_file_list()
    ...
    log2("read_and_index_file_list(%d, %d)", log_file_list_raw_idx, #log_file_list_raw)
    ...
    for i = 1, 5, 1 do
        ...
        lcd.drawText(5, 60, "indexing files: ", TEXT_COLOR + SMLSIZE)
        ...
        if modelName ~= nil then
            ...
            -- read file
            local start_time, end_time, total_seconds, total_lines, start_index, col_with_data_str = m_index_file.getFileDataInfo(fileName)

            if total_seconds ~= nil then
                --log3("read_and_index_file_list: total_lines: %s, total_seconds: %s, col_with_data_str: [%s]", total_lines, total_seconds, col_with_data_str)
                --log3("read_and_index_file_list: total_seconds: %s", total_seconds)
                if total_seconds > min_log_sec_to_show then
                    ...
                    log2("read_files_list: skipping short duration: %dsec, line: [%s]", total_seconds, fileName)
                end
            end
        end
...
local function filter_log_file_list(filter_model_name, filter_date)
    ...
    for i = 1, #log_file_list do
        ...
        --log2("filter_log_file_list: %d. %s", i, ln)
...
local function state_SELECT_FILE_refresh(event, touchState)
    ...
    if event == EVT_VIRTUAL_NEXT_PAGE then
        ...
        local start_time, end_time, total_seconds, total_lines, start_index, col_with_data_str
                = m_index_file.getFileDataInfo(filename)
