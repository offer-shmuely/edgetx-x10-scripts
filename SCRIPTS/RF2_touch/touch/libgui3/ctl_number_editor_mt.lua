...
function ctl_number_editor_mt(panel, id, args)
    panel.log("number_as_button ctl_number_editor_mt(): panel=%s, id=%s, [%s]=%s, min:%s, max:%s, steps:%s", panel, args.id, args.text, args.value, args.min, args.max, args.steps)
    ...
    --     panel.log("ctl_number_editor_mt onMeasureTapeValueChange: %s", obj.val)
    --     self.value = obj.val   --???
    -- end
    ...
    function self.covers(tsx, tsy)
        panel.log("ctl_number_editor_mt::covers() ?")
        if (tsx >= self.x and tsx <= self.x + self.w and tsy >= self.y and tsy <= self.y + self.h) then
            panel.log("ctl_number_editor_mt::covers() true")
            return true
        end

        panel.log("ctl_number_editor_mt::covers() - false")
        return false
    end

    function self.fullScreenRefresh()
        local x,y,w,h = self.x, self.y, self.w,self.h
        panel.log("ctl_number_editor_mt.fullScreenRefresh() - editing: %d", self.editing)
        ...
        if self.editing then
            -- menu background
            panel.log("ctl_number_editor_mt.fullScreenRefresh() EDITING")
