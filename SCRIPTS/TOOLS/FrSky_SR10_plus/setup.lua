...
chdir("/SCRIPTS/TOOLS/FrSky_SR10_plus")
...
local function getNextNilField(offset)
    for offsetIndex = offset or 1, #fields do
        if fields[offsetIndex][5] == nil and fields[offsetIndex][2] ~= T_HEADER then
...
local function redrawFieldsPage()
    ...
    for index = 1, numberPerPage, 1 do
        ...
        if is_simulator() and field[5] == nil then
            log("simu")
            if field[2] == T_VALUE then
                log("simu - value")
                field[5] = field[6]
            ...
            log("simu: %s=%s", field[1], field[5])
        end

        if field[2] == T_HEADER then
            attr = attr + BOLD
        end

        lcd.drawText(1, margin + spacing * index, field[1])

        if field[5] == nil and field[2] ~= T_HEADER then
            lcd.drawText(280, margin + spacing * index, "---", attr)
        else
            if field[2] == T_VALUE then
                lcd.drawText(280, margin + spacing * index, tostring(field[5]) .. field[8], attr)
            elseif field[2] == T_COMBO then
                if field[5] > 0 and field[5] <= #(field[6]) then
                    lcd.drawText(280, margin + spacing * index, field[6][field[5]], attr)
                end
            end
        end
    end
        ...
        if primId == 0x32 and dataId == 0x0C30 then
            ...
            for fieldIndex, thisField in ipairs(fields) do
                if fieldId == thisField[3] then
                    ...
                    if thisField[2] == T_COMBO and #thisField == 7 then
                    ...
                    elseif thisField[2] == T_VALUE and #thisField == 9 then
...
local function getFieldValue(field)
    ...
    if field[2] == T_COMBO and #field == 7 then
        value = field[7][value]
    elseif field[2] == T_VALUE and #field == 9 then
        value = value + field[9]
    end
    return value
end
