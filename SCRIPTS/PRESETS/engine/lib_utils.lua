...
-----------------------------------------------------------------
function M.drawTitle(txt, is_show, is_prev, is_next)
    --lcd.drawBitmap(ImgBackground, 0, 0)

    if is_show == true then
        lcd.drawFilledRectangle(1, 1, 480, 40, YELLOW) --LIGHTGREY
    end
...
function M.runConfigSummary(event)
    lcd.clear()
    ...
    drawTitle("Approve model change")
